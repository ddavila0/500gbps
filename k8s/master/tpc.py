...
class TransferTest:
  ...
  def checkSocket(source, destination):
    ...
    try:
      source_sock.connect((source, 2094))
      dest_sock.connect((destination, 2094))
      logging.info("Succesfully contacted socket 2094 on both sides")
  ...
  def makeTransferQueue(self):
    ...
    for num in range(self.numTransferStart, self.numTransferEnd):
      cmd = ['curl', '-L', '-X', 'COPY']
      cmd += ['-H', 'Overwrite: T']
      cmd += ['-H', f'Source: https://{self.source}:2094/testSourceFile{num}']
      cmd += [f'https://{self.destination}:2094/testDestFile{num}']
